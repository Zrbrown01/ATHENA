...
import {
  companionActions,
  nextCompanionAction,
  transitionCompanionRun,
  type CompanionAction,
  type CompanionStage,
} from "@/domain/companion/run";
import { companionFixture } from "@/domain/companion/fixture";
import {
  persistCompanionTransition,
  readCompanionRun,
} from "@/platform/companion-persistence";
import {
  GOLDEN_MATTER_ID,
  PILOT_TENANT_ID,
  pilotContext,
...
export async function GET(request: Request) {
  ...
  return Response.json({
    run,
    stage,
    nextAction: nextCompanionAction(stage),
    fixture: {
      matter: companionFixture.matter,
      document: companionFixture.document,
      facts: companionFixture.facts,
    },
  });
}
  ...
  try {
    ...
    if (body.action === "create_export") {
      await authorizeClassifiedUse({
        context: pilotContext(actor),
        tenantId: PILOT_TENANT_ID,
        matterId: GOLDEN_MATTER_ID,
        resourceType: "matter",
        resourceId: GOLDEN_MATTER_ID,
        plane: "export",
      });
    }
    ...
    const result = transitionCompanionRun(pilotContext(actor), {
      tenantId: PILOT_TENANT_ID,
      matterId: GOLDEN_MATTER_ID,
      runId: companionFixture.runId,
      currentStage,
      action: body.action,
      idempotencyKey: body.idempotencyKey,
    });
    await persistCompanionTransition({
    ...
    return Response.json({
      ...
      nextAction: nextCompanionAction(result.nextStage),
      event: result.event,
    });
  } catch (error) {
    ...
    if (error instanceof z.ZodError)
      return Response.json(
        { error: "Invalid workflow command", issues: error.issues },
        { status: 400 },
      );
    const message =
    ...
    return Response.json(
      { error: message },
      { status: message.includes("not valid") ? 409 : 500 },
    );
  }
}
...
export type CompanionResponse = {
  ...
  nextAction: CompanionAction | null;
  run: Awaited<ReturnType<typeof readCompanionRun>>;
};
