import { FileCheck2, FileClock, FileSearch, ShieldCheck } from "lucide-react";
import { AppShell } from "@/components/app-shell";
import { DocumentIntake } from "@/components/document-intake";
import { StatusPill } from "@/components/status-pill";
import { DocumentEvidenceOperations } from "@/components/document-evidence-operations";
...
export default function DocumentsPage() {
  return <AppShell active="Documents"><div className="simple-page documents-page">
    ...
    <DocumentIntake />
    <DocumentEvidenceOperations />
    <section className="panel document-table-panel" aria-labelledby="recent-documents"><header className="panel-heading"><div><h2 id="recent-documents">Recent documents</h2><p>Synthetic pilot fixtures. Originals remain distinct from OCR, summaries, redactions, and filing copies.</p></div></header><div className="work-table-wrap"><table className="document-table"><thead><tr><th>Document</th><th>Matter / account</th><th>Classification</th><th>Pages</th><th>Received</th><th>Status</th></tr></thead><tbody>{documents.map((document) => <tr key={document.title}><td><strong>{document.title}</strong><small>Original preserved · PDF</small></td><td>{document.matter}</td><td>{document.type}</td><td>{document.pages}</td><td>{document.received}</td><td><StatusPill tone={document.tone}>{document.status}</StatusPill></td></tr>)}</tbody></table></div></section>
  </div></AppShell>;
}
