...
export function PlatformOperations() {
  ...
  async function operate(action: "publish_internal" | "replay_dead_letters" | "run_internal_cycle") {
    ...
    try {
      ...
      setData(result);
      setMessage(action === "publish_internal" ? `${result.result.delivered ?? 0} internal events delivered.` : action === "replay_dead_letters" ? `${result.result.replayed ?? 0} dead letters replayed.` : "Internal delivery and reconciliation cycle completed.");
  ...
  return <div className="operator-grid">
    <section className="panel operator-panel"><header><div><h2>Transactional outbox</h2><p>{data.limitation}</p></div><StatusPill tone={data.outbox.deadLetter || data.reconciliation?.outcome === "exceptions" ? "danger" : data.outbox.pending ? "warning" : "success"}>{data.outbox.deadLetter || data.reconciliation?.outcome === "exceptions" ? "Attention" : data.outbox.pending ? "Ready" : "Healthy"}</StatusPill></header><dl><div><dt>Pending</dt><dd>{data.outbox.pending}</dd></div><div><dt>Leased</dt><dd>{data.outbox.leased}</dd></div><div><dt>Processed</dt><dd>{data.outbox.processed}</dd></div><div><dt>Dead letter</dt><dd>{data.outbox.deadLetter}</dd></div></dl><div className="operator-actions"><button className="primary-action" type="button" onClick={() => void operate("run_internal_cycle")} disabled={pending !== null}><RefreshCw size={15}/>Run delivery + reconcile</button><button className="secondary-action" type="button" onClick={() => void operate("replay_dead_letters")} disabled={pending !== null || data.outbox.deadLetter === 0}><ArchiveRestore size={15}/>Replay dead letters</button></div>{data.reconciliation&&<p>{data.reconciliation.detail} Processed {data.reconciliation.processedMessages}; checkpoints {data.reconciliation.checkpoints}; receipts {data.reconciliation.deliveryReceipts}; exceptions {data.reconciliation.exceptions}.</p>}{data.outbox.lastError && <p className="inline-error">{data.outbox.lastError}</p>}</section>
