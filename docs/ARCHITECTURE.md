...
Governed deadlines use immutable rule snapshots plus revisioned obligations, dependency edges, exception/waiver decisions, and escalation acknowledgements. Optimistic claims, the state mutation, immutable event, and outbox write share one D1 batch; a bounded SQLite-busy retry lets concurrent writers resolve without duplicating evidence.

Governance configuration is separately layered by firm, client, matter type, and matter scope. Resolution is deterministic and fail-closed on pending, expired, or unacknowledged content. Supersession atomically retires one active version and creates the next; simulation stores a point-in-time input, selection, and calculation snapshot without mutating operational obligations.

Full-matter obligation rebuild is a read-and-compare control. It traverses exact rule versions and dependency history, applies only approved exception/waiver evidence, and records matched, drifted, or blocked findings plus calculation traces. A partner reviews the immutable run under optimistic concurrency; no rebuild path writes to the operational obligation aggregate.

Referral opening and its initial deadlines share one D1 batch. An optimistic intake revision claim, matter state, two deterministic obligation snapshots, the intake decision, three immutable events, and three outbox records either all commit or all fail. Deterministic obligation identities and event idempotency prevent duplicate opening work; the historical pilot matter uses the same bundle through a one-time, revisioned migration command.
...
Directory reconciliation is provider-neutral at the snapshot boundary. The current adapter supplies a pinned Entra-shaped deterministic snapshot only; Athena compares normalized identities, enabled state, MFA claims, and active role codes, then stores the snapshot checksum, counts, drift findings, severity, blocking state, and revisioned partner review. Review batches use an optimistic claim so concurrent stale reviewers cannot duplicate the review/event/outbox evidence. The live adapter remains disabled until tenant consent, approved scopes, credentials, delta recovery, group mapping, session revocation, and provider reconciliation are available.
