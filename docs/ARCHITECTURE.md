...
Governed deadlines use immutable rule snapshots plus revisioned obligations, dependency edges, exception/waiver decisions, and escalation acknowledgements. Optimistic claims, the state mutation, immutable event, and outbox write share one D1 batch; a bounded SQLite-busy retry lets concurrent writers resolve without duplicating evidence. 6. **Integration control plane:** provider connections, subscriptions, cursors, idempotency, retries, dead letters, backfill, reconciliation, and revocation.
