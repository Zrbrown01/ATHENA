...
Recovery tests prove exact required-category verification, mismatched restore failure, verification-before-approval, and tenant denial. The repeatable `npm run db:recovery:verify` exercise exports local D1, restores into disposable SQLite, and currently matches 180 application tables, 47 migrations through `0046_wonderful_lester.sql`, 283 events, 283 outbox rows, the pinned export SHA-256 `7e994d0b540d8b0ca0ff535e7ba345f60e870452f981006e4ca307ad411ed56d`, one tenant, integrity `ok`, and zero foreign-key violations without production mutation.
