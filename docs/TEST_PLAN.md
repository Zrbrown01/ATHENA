...
Client-instruction tests pin the deterministic PDF byte size and SHA-256, require source-integrity and instruction-scope approvals from distinct authenticated reviewers, reject incomplete/stale/cross-tenant commands, and prove synthetic activation never claims verified external content. The API/browser proof races two activation requests and two deactivation requests, requires one success and one 409 at each boundary, verifies idempotent winner replay, and confirms that only the exact checksum-linked active client layer influences deterministic policy simulation.

Recovery tests prove exact required-category verification, mismatched restore failure, verification-before-approval, and tenant denial. The repeatable `npm run db:recovery:verify` exercise exports local D1, restores into disposable SQLite, and currently matches 195 application tables, 53 migrations through `0052_even_archangel.sql`, 509 events, 509 outbox rows, the pinned export SHA-256 `a769ba4d03f8543e54c57e81bcb6d728b889a6902a7ba05b075a06a3dc5c594a`, one tenant, integrity `ok`, and zero foreign-key violations without production mutation.
