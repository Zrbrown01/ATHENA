...
Tenant-portability tests prove that missing schema tables or original objects force `partial`, completeness requires full coverage, partner role and tenant isolation are enforced, and every declared archive entry is read back with exact byte-size/SHA-256 verification. The API proof built and downloaded a 281,600-byte TAR with 11 entries, 12 coverage categories, 32 of 150 tables, zero available originals, a matching archive SHA-256, the explicit `118_schema_tables_not_serialized` gap, and a foreign-tenant 403.
