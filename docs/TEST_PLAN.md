...
The route-security contract discovers all 80 exported handlers across 43 API route files and fails when any handler omits authenticated identity or an authorization boundary, or when any mutation omits trusted-origin enforcement or durable rate limiting. Pilot identity tests prove that an unknown production principal receives no roles or matter access, only an explicitly configured owner receives attorney/partner access, and a configured support principal remains outside ordinary matter membership. Axe-core runs every DOM-computable rule against the global shell and the initial client-portal, scale-evidence, and tenant-export surfaces; color contrast is excluded because jsdom has no rendered style/layout engine and remains a real-browser/manual gate.
