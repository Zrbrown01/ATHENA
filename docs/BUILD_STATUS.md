...
| Proceedings/readiness | Functional settlement and proceeding slices | Settlement snapshots use current graph/work state. A separate durable proceeding lifecycle covers all required proceeding types and twelve readiness areas, with revisioned evidence, immutable snapshots/findings, six readiness states, idempotent event/outbox history, and honest human-verified external filing mode. Versioned forms/signatures/attachments and attorney-approved California content remain; EAMS is disconnected. |
