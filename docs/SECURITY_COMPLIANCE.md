...
- Tenant export creation is partner/security-admin restricted, checks every persisted matter through the export data plane, dynamically includes only tables with an explicit tenant column, uses server-selected object keys, and preserves archive/category/checksum/decision/event evidence. Any uncovered table or original forces `partial`; provider-held data and independent database restoration remain separate gates.
